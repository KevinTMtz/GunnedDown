...
public class HydraShoot : MonoBehaviour
{
    ...
    void rotateFirePoint()
    {
        ...
        {
            Vector2 aim = firePoints[i].transform.position - new Vector3(target.transform.position.x+Random.Range(-2.5f,2.5f), target.transform.position.y + Random.Range(-2.5f, 2.5f),0);
    ...
    public void CreateObstacles()
    {
        ...
        for(int i=obstacleOffset+2; i <= obstacleOffset*numberArms+2; i=i+obstacleOffset)
        {
            if (xObstacle) Instantiate(obstacle, transform.position + new Vector3(i, 0, 0), Quaternion.identity);
            else Instantiate(obstacle, transform.position + new Vector3(-i, 0, 0), Quaternion.identity);
            if (yObstacle) Instantiate(obstacle, transform.position + new Vector3(0, i, 0), Quaternion.identity);
            else Instantiate(obstacle, transform.position + new Vector3(0, -i, 0), Quaternion.identity);
        }
        ...
        if (!gameObject.GetComponent<HydraHealth>().scream2)
        {
            ...
            {
                Instantiate(obstacle, target.transform.position + new Vector3(Random.Range(-10, 10), Random.Range(-10, 10), 0), Quaternion.identity);
            }
        }
